...
if USE_DB:
    ...
    def init_db():
        with db() as cur:
            ...
            """)
            # 기존 테이블에 section 컬럼이 없으면 추가
            cur.execute("""
                ALTER TABLE entries ADD COLUMN IF NOT EXISTS section TEXT DEFAULT 'promo'
            """)
...
def load_data(team=None, section=None):
    if USE_DB:
        conditions, params = [], []
        if team:
            conditions.append("team = %s"); params.append(team)
        if section:
            conditions.append("section = %s"); params.append(section)
...
def get_data():
    ...
    return jsonify(load_data(team=team, section=section))
...
def export():
    ...
    title_text = "신제품 체험단 일정 취합본" if section == "tasting" else "제품 출시 및 프로모션 일정 취합본"
